...
$stmt->close();
if(!$subject){ echo "<div>Subject not found</div>"; include_once __DIR__.'/../inc/footer.php'; exit; }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!$subject_name) $errors[]="Subject name required.";
    if(empty($errors)){
        ...
        if($class_id){
            ...
            $null=null;
            $stmt->bind_param('sii',$subject_name,$null,$id);
        }
        if($stmt->execute()) $success="Subject updated.";
        else $errors[]="Error updating subject.";
        $stmt->close();
    }
}
?>
...
<div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
  ...
  <form method="post" class="space-y-4">
    <div>
      ...
      <input name="subject_name" value="<?= htmlspecialchars($subject['subject_name']) ?>" class="mt-1 block w-full rounded-md border-gray-200 px-3 py-2" required>
    </div>
    <div>
      ...
      <select name="class_id" class="mt-1 block w-full rounded-md border-gray-200 px-3 py-2">
        ...
        <?php while($c=$classes->fetch_assoc()): ?>
          <option value="<?= $c['class_id'] ?>" <?= $subject['class_id']==$c['class_id']?'selected':'' ?>>
